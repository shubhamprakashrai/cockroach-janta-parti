...
export default function BlogIndexPage() {
    ...
    return (
        <main className="min-h-screen bg-bg text-text-primary pb-24">
            {/* Navbar */}
            <nav className="sticky top-0 z-50 bg-card border-b-4 border-text-primary px-4 py-4 shadow-[0_4px_0_0_#FFF] flex justify-between items-center">
            ...
            <header className="px-4 py-16 text-center border-b-4 border-text-primary bg-bg relative">
                <div className="max-w-4xl mx-auto">
                    <h1 className="font-display text-5xl md:text-7xl uppercase text-white mb-8">THE ROACH <span className="text-accent underline decoration-8">DIGEST</span></h1>
                    <div className="relative max-w-2xl mx-auto mb-8">
                        ...
                        <input
                            ...
                            className="w-full bg-card border-4 border-text-primary p-5 pl-14 font-mono font-bold uppercase text-white shadow-[8px_8px_0_0_#FFD60A] focus:outline-none focus:border-accent"
                        />
                    </div>
                    ...
                    {posts.map((post, i) => (
                        <Link href={`/blog/${post.slug}`} key={i} className="flex flex-col bg-card border-4 border-text-primary shadow-[12px_12px_0_0_#000] hover:shadow-[12px_12px_0_0_#FFD60A] hover:-translate-y-2 transition-all group overflow-hidden">
                            ...
                            <div className="p-6 flex-1 flex flex-col">
                                <h2 className="font-display text-3xl uppercase text-white mb-3 leading-tight group-hover:text-accent transition-colors">{post.title}</h2>
